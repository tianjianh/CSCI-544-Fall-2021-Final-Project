...
def get_dist_mask_tile(x, y):
    mask = torch.FloatTensor(x, y).to(DEVICE)
    for i in range(x):
        for j in range(y):
            mask[i, j] = -abs(i-j)
    mask.unsqueeze_(0)
...
def attention(query, key, value, mask=None, dropout=None):
    ...
    _, _, x, y = scores.shape

    dist_mask_tile = get_dist_mask_tile(x, y)

    scores += dist_mask_tile
...
def evaluate(data, model):
    ...
    f_truth = open("truth.txt", "w", encoding="utf-8")
    f_pred = open("pred.txt", "w", encoding="utf-8")
    with torch.no_grad():
        # 在data的英文数据长度上遍历下标
        for i in range(len(data.dev_en)):
            ...
            f_truth.write("".join(cn_sent) + "\n")
            f_pred.write("".join(translation) + "\n")
